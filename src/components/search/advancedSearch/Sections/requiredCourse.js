...
export default function RequiredCourses({onUpdate, formData, filterChips}) {
    const requiredCourses = formData?.requiredCourses ?? [{}];
    ...
    const handleFieldChange = (index, field, value) => {
        ...
        onUpdate({requiredCourses: updatedCourses});
        filterChips(prev => ({
            ...prev,
            courseRequirements: {
    ...
    const handleRemoveRequirement = index => {
        const updatedCourses = requiredCourses.filter((_, i) => i !== index);
        onUpdate({requiredCourses: updatedCourses});
    };
                                ...
                                <SelectField
                                    ...
                                    options={subjectOptions.map(
                                        (option, idx) => ({
                                            ...
                                            key: `subjecto-${option.courseNumber}-${idx}`
                                        })
                                    )}
                                    onChange={val =>
                                ...
                                {isSubstituteCourse[index] ? (
                                    ...
                                    >
                                        <SelectField
                                            ...
                                            options={substituteCourseOptions.map(
                                                (option, idx) => ({
                                                    label: option.subject,
                                                    value: option.subject,
                                                    key: `substitutes-${option.courseNumber}-${idx}`
                                                })
                                            )}
                                            onChange={val =>
                                                handleFieldChange(
